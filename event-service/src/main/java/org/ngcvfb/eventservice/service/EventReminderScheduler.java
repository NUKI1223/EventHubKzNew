...
/**
 * Периодически рассылает напоминания о приближающихся мероприятиях тем, кто их лайкнул.
 * <p>
 * Источник правды по лайкам — like-service, по email — user-service (через Feign).
 * Каждое срабатывание отправляет события в Kafka-топик {@code event.reminder};
 * дедупликация на стороне notification-service ({@code createIfAbsent}) гарантирует,
 * что пользователь получит ровно одно напоминание на событие, даже при повторных прогонах.
 */
@Component
...
public class EventReminderScheduler {
    ...
    private void remindForEvent(Event event) {
        List<Map<String, Object>> likes = likeClient.getLikesByEvent(event.getId());
        ...
        Map<Long, String> emailById = userClient.getUsersByIds(userIds).stream()
